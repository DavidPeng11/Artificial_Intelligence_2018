...
print("train : " + str(orig_train.shape))

orig_train = orig_train[orig_train.GrLivArea < 4000]
...
# print(train_test_combine.loc[:, "SalePrice"].isnull().values.sum())
# print(train_test_combine.loc[:, "SalePrice"].value_counts())
...
print(train.loc[:, "OverallQual"].value_counts())
...
new_test = train[train["SalePrice"] == 0]
new_train = train[train["SalePrice"] != 0]

y_train = new_train.SalePrice
X_train = new_train[new_train.columns.drop("SalePrice")]
X_test = new_test[new_test.columns.drop("SalePrice")]

test_Id = new_test.Id
X_train = X_train[X_train.columns.drop("Id")]
X_test = X_test[X_test.columns.drop("Id")]
...
for i in range(1461, 2919):
  if i not in list(test_Id):
    print(i)
...
output_result = pd.DataFrame({"Id":test_Id, "SalePrice":y_test_pred})
output_result.to_csv('linear_regression.csv', index=False)